...
class WaveFile:
    ...
    def __init__(self, path: str):
        ...
        self.loudest_samp_value: Union[int, None] = None  # value of loudest sample in the signal
        self.max_possible_samp_value: Union[int, float, None] = None
    ...
    def load_signal(self, path: str) -> None:
        ...
        self.loudest_samp_value = min_sample_value if np.abs(min_sample_value) > max_sample_value else max_sample_value

        if np.issubdtype(self.input_signal.dtype, np.integer):
            self.max_possible_samp_value = np.iinfo(self.input_signal.dtype).max
        else:
            self.max_possible_samp_value = 1.0  # assuming that all other cases are floats
        ...
        # and normalize it by the sum of window samples (here its a rectangle, therefore the sum equals its length)
        scaled_amp_values: ndarray = 2 * self.amplitudes / len(self.amplitudes)
        # convert to dBFS (decibels full scale)
        self.amplitudes_db = 20 * np.log10(scaled_amp_values / self.max_possible_samp_value)
